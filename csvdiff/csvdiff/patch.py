...
#Denne filen er redigert for å matche kravspesifikasjoner.
...
def _assemble(removed, added, changed, from_recs, to_recs, index_columns):
    ...
    diff['changed'] = sorted(({'nøkkel': list(k),
                               'felt': record_diff(from_recs[k], to_recs[k])}
                              for k in changed),
                             key=_change_key)
    return diff
...
def filter_significance(diff, significance):
    ...
    reduced = [{'nøkkel': delta['nøkkel'],
                'felt': {k: v
                           for k, v in delta['felt'].items()
                           if _is_significant(v, significance)}}
               for delta in changed]

    # call a key changed only if it still has significant changes
    filtered = [delta for delta in reduced if delta['felt']]
